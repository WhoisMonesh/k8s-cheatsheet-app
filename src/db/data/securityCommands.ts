...
backend     allow-frontend   app=backend    2d`,
  ...
  // Secret Creation
  {
    ...
    subcategory: "Secrets Management",
    command:
      "kubectl create secret docker-registry my-registry --docker-server=DOCKER_REGISTRY_SERVER --docker-username=DOCKER_USER --docker-password=DOCKER_PASSWORD --docker-email=DOCKER_EMAIL",
    description: "Create a docker-registry secret",
    example:
      "kubectl create secret docker-registry my-registry --docker-server=https://index.docker.io/v1/ --docker-username=myuser --docker-password=mypassword --docker-email=myuser@example.com",
    versionIntroduced: "1.0",
    difficultyLevel: "intermediate",
