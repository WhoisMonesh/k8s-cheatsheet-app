...
export function ConsolePractice() {
  const [selectedExercise, setSelectedExercise] = useState<ConsoleExercise>(consoleExercises[0]);
  const [currentTaskIndex, setCurrentTaskIndex] = useState(0);
  const [terminalHistory, setTerminalHistory] = useState<string[]>(['Welcome to Kubernetes Practice Console', 'Type "help" for available commands.']);
  ...
  const handleCommand = (e: React.FormEvent) => {
    ...
    if (cmd === 'clear') {
      ...
      output = 'Available commands: kubectl, clear, help. Try to solve the current task!';
    } else if (currentTask) {
      ...
      if (regex.test(cmd)) {
        ...
        if (cmd.startsWith('kubectl')) {
           output = 'Error: Incorrect command or arguments for the current task. Check the hint!';
        } else {
      ...
      <div className="w-1/3 flex flex-col gap-6">
        <div className="bg-slate-800 rounded-xl border border-slate-700 p-4 flex-1 overflow-y-auto">
          ...
          <div className="border-t border-slate-700 pt-4">
             <h3 className="text-lg font-semibold text-white mb-2">{selectedExercise.title}</h3>
                    ...
                    return (
                        ...
                        >
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <div className={`text-sm font-medium ${isDone ? 'text-green-400' : 'text-slate-200'}`}>
                                        Task {idx + 1}
                                    </div>
                                    <div className="text-sm text-slate-300 mt-1">{task.text}</div>
                                    {isActive && (
      ...
      <div className="w-2/3 flex flex-col bg-[#1e1e1e] rounded-xl border border-slate-700 shadow-2xl overflow-hidden font-mono">
        ...
        <div className="flex-1 p-4 overflow-y-auto text-sm" onClick={() => document.getElementById('terminal-input')?.focus()}>
          {terminalHistory.map((line, i) => (
            <div key={i} className={`${line.startsWith('$') ? 'text-white font-bold mt-2' : line.startsWith('✅') ? 'text-green-400 my-1' : 'text-slate-300'} whitespace-pre-wrap`}>
              {line}
            </div>
          ))}
