...
export function YamlBuilder() {
  ...
  const [activeTab, setActiveTab] = useState<"basic" | "spec" | "advanced">(
    "basic",
  );
...
${ports
  .map(
    (p) => `${indent}  - containerPort: ${p.containerPort}
...
${indent}    mountPath: ${v.mountPath}`,
  ...
  .join("\n")}`
    ...
    const volumesSpec = (indent: string) =>
      volumes.length > 0
        ? `
${indent}volumes:
${volumes
  .map(
    (v) => `${indent}- name: ${v.name}
${indent}  ${v.type}:
${indent}    ${v.type === "emptyDir" ? "{}" : v.type === "pvc" ? `claimName: ${v.source}` : `path: ${v.source}`}`,
  )
  .join("\n")}`
        : "";
...
${servicePorts
  .map(
    ...
    protocol: ${p.protocol}
${p.nodePort && serviceType === "NodePort" ? `    nodePort: ${p.nodePort}` : ""}`,
  )
  .join("\n")}`;
        break;
                  ...
                  {form.labels.map((label, idx) => (
                    <div key={idx} className="flex gap-2 mb-2">
                      <input
                        ...
                        value={label.key}
                        onChange={(e) =>
                          updateArrayItem("labels", idx, "key", e.target.value)
                        }
                      ...
                      />
                      <button
                        onClick={() => removeArrayItem("labels", idx)}
                      ...
                      {resourceType === "deployment" && (
                        <div>
                          ...
                          <input
                            ...
                            value={form.replicas}
                            onChange={(e) =>
                              updateForm("replicas", parseInt(e.target.value))
                            }
                            className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                          />
                        </div>
                      ...
                      {resourceType === "cronjob" && (
                        <div>
                          ...
                          <input
                            value={form.schedule}
                            onChange={(e) =>
                              updateForm("schedule", e.target.value)
                            }
                            className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                            placeholder="*/5 * * * *"
                          />
                  ...
                  <>
                    <div className="mb-4">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          updateForm("serviceType", e.target.value)
                        }
                        className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                      >
                    ...
                    <div>
                      ...
                      {form.servicePorts.map((port, idx) => (
                        <div key={idx} className="flex gap-2 mb-2 items-center">
                          ...
                          <button
                            onClick={() => removeArrayItem("servicePorts", idx)}
                            className="text-red-500 hover:text-red-700"
                          >
                            <Trash2 className="w-4 h-4" />
                          </button>
                    ...
                    {form.data.map((item, idx) => (
                      <div key={idx} className="flex gap-2 mb-2">
                        <input
                          ...
                          value={item.key}
                          onChange={(e) =>
                            updateArrayItem("data", idx, "key", e.target.value)
                          }
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateNested("resources", "requests", {
                            ...form.resources.requests,
                            cpu: e.target.value,
                          })
                        }
                        className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateNested("resources", "limits", {
                            ...form.resources.limits,
                            cpu: e.target.value,
                          })
                        }
                        className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
