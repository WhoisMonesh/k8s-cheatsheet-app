  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Workflow Scenarios</h2>
          <p className="text-slate-600 dark:text-slate-400 mt-1">Step-by-step guides for common Kubernetes operations</p>
        </div>
      </div>
      
      <div className="grid gap-4">
        {scenariosData.map(scenario => (
          <div key={scenario.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden transition-all duration-200 hover:shadow-md">
            <button
            ...
            >
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">{scenario.title}</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400">{scenario.description}</p>
              </div>
              <div className="flex items-center gap-4">
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wide ${
                  scenario.difficulty === 'beginner' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400' :
                  scenario.difficulty === 'intermediate' ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400' :
                  'bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-400'
                }`}>
                  {scenario.difficulty}
                </span>
                {expandedId === scenario.id ? <ChevronDown className="w-5 h-5 text-slate-400" /> : <ChevronRight className="w-5 h-5 text-slate-400" />}
              </div>
            </button>
                  ...
                  {scenario.steps.map((step, index) => (
                    <div key={index} className="relative pl-10 pb-8 last:pb-0">
                      ...
                      <div className="bg-white dark:bg-gray-900 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
                        <h4 className="font-bold text-slate-900 dark:text-white mb-3 text-lg">{step.description}</h4>
